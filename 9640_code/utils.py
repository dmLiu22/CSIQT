...
@torch.no_grad()
def update_teacher_model(model, model_teacher, keep_rate=0.9990):
    ...
    # print(student_model_dict.keys())
    new_teacher_dict = OrderedDict()
    for key, value in model_teacher.state_dict().items():
        if key[7:] in student_model_dict.keys(): #key[7:]:cls_token
            new_teacher_dict[key] = (
                student_model_dict[key[7:]] *
                (1 - keep_rate) + value * keep_rate
            )
        else:
            new_teacher_dict[key] = value
    model_teacher.load_state_dict(new_teacher_dict, strict=False)

@torch.no_grad()
def copy_main_model1(model, model_teacher):
    ...
    if dist.get_world_size() > 1:
        ...
        model_teacher.load_state_dict(self.model.state_dict())
