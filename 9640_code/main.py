...
def main(config):
    ...
    # model_saliency = SODModel()
    # chkpt = torch.load('/home/data/hrz/ACM_MM/Saliency/best-model_epoch-204_mae-0.0505_loss-0.1370.pth', map_location='cpu')
    # model_saliency.load_state_dict(chkpt['model'])
    # model_saliency.cuda()
    # model_saliency.eval()
    ...
    # model_saliency = torch.nn.parallel.DistributedDataParallel(
    #     model_saliency,
    #     device_ids=[config.LOCAL_RANK],
    #     broadcast_buffers=False,
    #     find_unused_parameters=True,
...
def train_one_epoch(
    ...
    for idx, (samples, _, targets) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE):
            ...
            with torch.no_grad():
                # if config.TRAIN.START_EPOCH<3:
                # # pred_masks, _ = model_saliency(samples)
                #     copy_main_model1(model,t_model)
                # else:
                #     update_teacher_model(model, t_model)
                # output_t = t_model(samples)

                output_t = t_model(samples)
                # output_t, _, _, _, _ = t_model(samples)
                # output_t, _ = t_model(samples,_ ,_)

            outputs, con_loss = model(samples, targets, output_t.detach())
            # update_teacher_model(model, t_model)

        targets.unsqueeze_(dim=-1)
        # 修改10
        loss_Soft = criterion(outputs, output_t.detach())  # SmoothL1Loss 损失函数
        loss_GT = criterion(outputs, targets)  # SmoothL1Loss 损失函数
        loss = loss_GT + con_loss + 0.5 * loss_Soft
        loss = loss / config.TRAIN.ACCUMULATION_STEPS
        #
        # pred_scores = pred_scores + (outputs + aux_score).squeeze().cpu().tolist()
        pred_scores = pred_scores + (outputs).squeeze().cpu().tolist()  # 用cls token 作输出
        ...
        loss_GT_meter.update(loss_GT.item(), targets.size(0))
        con_loss_meter.update(con_loss.item(), targets.size(0))
        ...
        if idx % config.PRINT_FREQ == 0:
            ...
            logger.info(
                ...
                f"loss_all {loss_meter.val:.4f} ({loss_meter.avg:.4f})\t"
                f"loss_GT {loss_GT_meter.val:.4f} ({loss_GT_meter.avg:.4f})\t"
                f"con_loss {con_loss_meter.val:.4f} ({con_loss_meter.avg:.4f})\t"
